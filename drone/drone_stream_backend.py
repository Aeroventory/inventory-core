"""
Standalone FastAPI stream/control runtime for the HTJR Wi-Fi drone.

Dependencies for the destination app:
    pip install fastapi uvicorn

This file intentionally does not import fixed_drone.py or auto_mission.py.
It reimplements the working RTSP/RTP/JPEG repair and 13-byte control packet.
"""
...
class DroneRuntime:
    ...
    def capture_photo(self) -> str:
        ...
        path = os.path.join(self.config.capture_dir, f"htjr_{stamp}_{int(time.time() * 1000)}.jpg")
...
app = FastAPI(title="HTJR Drone Stream Backend")
...
def mjpeg_frame_generator() -> Iterator[bytes]:
    ...
    boundary = b"--frame\r\n"
    while True:
        item = runtime.wait_for_frame(last_seen=last_seen, timeout=5.0)
        if item is None:
            continue
        last_seen, jpeg = item
        yield (
...
@app.get("/api/drone/stream.mjpg")
async def drone_mjpeg_stream() -> StreamingResponse:
    try:
...
@app.websocket("/api/drone/ws/stream")
