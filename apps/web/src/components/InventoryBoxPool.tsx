...
import { Boxes, RefreshCw, RotateCcw, Save, Trash2, X } from "lucide-react";
...
interface InventoryBoxPoolProps {
  ...
  embedded?: boolean;
  refreshKey?: number;
  stagedRemovedBoxIds?: number[];
  onStagedRemovedBoxIdsChange?: (boxIds: number[]) => void;
  onChanged?: () => void;
...
export default function InventoryBoxPool({
  ...
  const stagedRemovedSet = useMemo(() => new Set(stagedRemovedBoxIds), [stagedRemovedBoxIds]);
  const captureBoxes = useMemo(
    () => boxes.filter((box) => !stagedRemovedSet.has(box.id)),
    [boxes, stagedRemovedSet],
  );
  const captureUnitCount = captureBoxes.reduce((sum, box) => sum + box.quantity, 0);
  ...
  const loadData = async () => {
    ...
    try {
      const [boxRes, productRes] = await Promise.all([
        getInventoryBoxes({ active: true }),
        getProducts(),
      ]);
      setBoxes(boxRes.data);
      setProducts(productRes.data);
    } catch {
      const message = t("snapshots.boxes.errors.load");
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    onStatsChange?.({
      activeBoxCount: captureBoxes.length,
      activeUnitCount: captureUnitCount,
    });
  }, [captureBoxes, captureUnitCount, onStatsChange]);
  ...
  const handleCreate = async () => {
    ...
    if (!productId || quantity <= 0 || !draft.box_date) {
      const message = t("snapshots.boxes.errors.invalid");
      setError(message);
      toast.error(message);
      return;
    }
    ...
    try {
      ...
      await loadData();
      onChanged?.();
      toast.success(t("snapshots.boxes.success.create"));
    } catch {
      const message = t("snapshots.boxes.errors.save");
      setError(message);
      toast.error(message);
    } finally {
      setSaving(false);
  ...
  const handleSaveEdit = async (boxId: number) => {
    ...
    try {
      ...
      await loadData();
      onChanged?.();
      toast.success(t("snapshots.boxes.success.update"));
    } catch {
      const message = t("snapshots.boxes.errors.save");
      setError(message);
      toast.error(message);
  ...
  const markForRemoval = (boxId: number) => {
    setEditBoxId((current) => (current === boxId ? null : current));
    if (stagedRemovedSet.has(boxId)) return;
    onStagedRemovedBoxIdsChange?.([...stagedRemovedBoxIds, boxId]);
  };

  const restoreBox = (boxId: number) => {
    onStagedRemovedBoxIdsChange?.(stagedRemovedBoxIds.filter((id) => id !== boxId));
  };
    ...
    <div className="space-y-5">
      {embedded ? (
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h3 className="text-base font-medium text-[#10231B]">{t("snapshots.boxes.title")}</h3>
            <p className="mt-1 text-sm text-[#5B6B63]">{t("snapshots.boxes.description")}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge tone={captureBoxes.length ? "blue" : "neutral"}>
              {t("common.formats.boxes", { count: captureBoxes.length })}
            </Badge>
            <Badge tone={captureUnitCount ? "green" : "neutral"}>
              {t("common.formats.units", { count: captureUnitCount })}
            </Badge>
            <Button variant="secondary" onClick={() => void loadData()} disabled={loading}>
              <RefreshCw size={16} className={loading ? "animate-spin" : undefined} />
              {t("common.actions.refresh")}
      ...
      {isAdmin && (
        <div className="grid gap-4 rounded-xl border border-[#D9E4DD] bg-[#F7FAF8] p-4 lg:grid-cols-[1.2fr_1fr_0.8fr_0.9fr_auto]">
          <Input
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-[#D9E4DD] bg-white">
          <table className="w-full min-w-[900px] border-separate border-spacing-0 text-left text-sm">
            ...
            <tbody className="bg-white">
              {boxes.map((box) => {
                const isMarkedForRemoval = stagedRemovedSet.has(box.id);
                const isEditing = !isMarkedForRemoval && editBoxId === box.id;
                return (
                  <tr key={box.id} className={isMarkedForRemoval ? "bg-[#FFF9E6]" : "hover:bg-[#F7FAF8]"}>
                    ...
                    <td className="border-b border-[#D9E4DD] px-4 py-3">
                      <Badge tone={isMarkedForRemoval ? "warning" : "blue"}>
                        {isMarkedForRemoval
                          ? t("snapshots.boxes.status.markedForRemoval")
                          : t("snapshots.boxes.status.active")}
                      </Badge>
                    </td>
                    <td className="border-b border-[#D9E4DD] px-4 py-3">
                      {isAdmin && (
                        <div className="flex flex-wrap gap-2">
                          {isEditing ? (
                            ...
                            <Button size="sm" variant="soft" onClick={() => restoreBox(box.id)}>
                              <RotateCcw size={15} />
                            ...
                            <>
                              ...
                              <Button size="icon" variant="danger" aria-label={t("common.actions.delete")} onClick={() => markForRemoval(box.id)} disabled={saving}>
                                <Trash2 size={15} />
                              </Button>
                            </>
                          )}
