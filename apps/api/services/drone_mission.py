...
class MissionController:
    def __init__(self, runtime: DroneRuntime) -> None:
    ...
    def _run(self, steps: list[MissionStep]) -> None:
        try:
            ...
            self.runtime.neutral_for(0.30, "mission warmup")
            for step in steps:
    ...
    def _execute_step(self, step: MissionStep) -> None:
        ...
        if command == "takeoff":
            ...
            self.runtime.send_for(
                HtjrPacketState(rudd=128 - self.runtime.config.axis_delta),
                require_seconds(step),
                "yaw_left",
            )
